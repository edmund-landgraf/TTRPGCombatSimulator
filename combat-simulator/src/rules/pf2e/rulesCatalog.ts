...
export const RULES_CATALOG: RulesEntry[] = [
  ...
  },
  {
    ...
    verbose: [
      ...
      "Draw center → center: creature on the line gives the target lesser cover (+1 AC); terrain/object on the line or defender on cover gives standard cover (+2 AC).",
      "Intervening creature ≥2 size categories larger than both attacker and target → standard cover (+2 AC).",
      "No general chance to hit your ally — only the target’s AC rises.",
  ...
  {
    id: "Lesser_cover",
    aliases: ["lesser_cover", "creature_cover", "soft_cover", "shoot_through"],
    title: "Lesser cover (creature on attack line)",
    actions: ["Strike_ranged", "Cast_cantrip", "Cast_spell"],
    brief: "When a ranged attack line passes through another creature, the target gains +1 circumstance to AC.",
    verbose: [
      "PF2e Cover: draw a line from the center of the attacker’s space to the center of the target’s space.",
      "If the line passes through another creature (not the attacker or target), the target has lesser cover (+1 AC).",
      "If that creature is at least two size categories larger than both attacker and target, use standard cover (+2 AC) instead.",
      "There is no general rule that the shot might hit the intervening creature — only AC increases.",
      "Penetrating Shot / Penetrating Fire are special feats that shoot through an intervening foe (not modeled by default).",
    ],
    simStatus: "implemented",
  },
  {
    id: "Switch_weapon",
    aliases: ["switch_weapon", "switch", "draw", "stow", "change_weapon"],
    title: "Switch Weapon",
    actions: ["Switch_weapon"],
    brief: "1 action. Change the weapon you are wielding. Strikes use the held weapon only.",
    verbose: [
      "Sim simplification: switching costs 1 action (full PF2e often uses Interact to draw/stow).",
      "Each combatant starts with a defaultWeaponId (or an inferred default) in hand.",
      "You may only Strike with the held weapon; other carried weapons require a switch first.",
      "Threatened squares and Reactive Strike use the held melee weapon’s reach.",
    ],
    simStatus: "implemented",
  },
  {
  ...
  {
    id: "aoe_terrain",
    aliases: ["aoe", "area", "grease", "fog", "fog_cloud", "blast", "fireball", "leave_terrain"],
    title: "Area spells & terrain marks",
    actions: ["Cast_spell", "Cast_cantrip"],
    brief:
      "Square/burst AoE can hit multiple foes and optionally paint lasting terrain (Grease → G, Fog Cloud → F).",
    verbose: [
      ...
      "leaveTerrain + terrainGlyph: paint tag onto walkable cells (grease → G, fog → F); pathing treats grease as difficult.",
      "Fog is not a −2 attack penalty: creatures in fog are concealed (DC 5 flat check before Strikes / spell attacks).",
      ...
      "ASCII empty grease/fog cells show G/F; tokens still draw on top when occupying those cells.",
    ],
    simStatus: "partial",
  },
  {
    id: "concealment_fog",
    aliases: ["concealed", "concealment", "fog", "mist", "fog_cloud"],
    title: "Fog & concealed",
    actions: ["Strike_melee", "Strike_ranged", "Cast_spell", "Cast_cantrip"],
    brief:
      "Fog terrain conceals creatures (DC 5 flat check to target). Not a −2 circumstance penalty to attack rolls.",
    verbose: [
      "PF2e: mist/fog grants the concealed condition — before you roll an attack or single-target effect, attempt a DC 5 flat check.",
      "Fail the flat check → you miss / don't affect the target; the action (and MAP for Strikes) is still spent.",
      "Sim: if the attacker or the target is in a fog cell, the check applies (Fog Cloud mutual concealment model).",
      "Area effects skip the flat check. Cover (+2 AC) is separate from concealment.",
    ],
    simStatus: "partial",
  },
  {
  ...
  },
  {
    ...
    verbose: [
      ...
      "Sim status: Stride leaving a threatened square triggers Reactive Strike (Step does not). Reach 2 expands control. Ranged-attack triggers not modeled yet.",
    ],
    simStatus: "partial",
  },
  {
    ...
    verbose: [
      ...
      "Until ranged-attack triggers are modeled, long weapons mainly affect Strike range + Stride provokes.",
    ],
    simStatus: "partial",
  },
  {
  ...
  },
  {
    ...
    verbose: [
      ...
      "Feat lookup: npm run ingest-feats → data/feats/{index,catalog}.json; shouldAttemptFeat gates Step 4 score hints.",
    ],
    simStatus: "partial",
  },
  {
    ...
    verbose: [
      ...
      "Archetype tags normalize via data/archetypes/catalog.json (npm run ingest-archetypes); dedications grant capabilities + overlays.",
      ...
      "Feat catalog: ingest-feats builds full AoN ID index + narrow combat stubs; shouldAttemptFeat gates Step 4 prefer/score hints.",
      "Afflictions: ingest-afflictions (trait_group:Affliction); exposeAffliction on weapon hit / spell fail; EOT stage saves.",
    ],
    simStatus: "partial",
  },
  {
    id: "afflictions",
    aliases: ["poison", "disease", "curse", "venom", "virulent"],
    title: "Afflictions (poison, disease, curse)",
    actions: [],
    brief:
      "Initial Fort save → stages with damage/conditions → interval saves reduce or advance stage; poisons re-expose advances stage.",
    verbose: [
      "Catalog stubs in data/afflictions/catalog.json (Giant Centipede Venom, Arsenic, …).",
      "Weapon.afflictionId / Spell.applyAffliction expose on hit or failed save.",
      "Virulent: two consecutive successes to reduce one stage; crit success reduces by 1 only.",
      "Max duration ends the affliction; disease/curse ignore duplicate exposures.",
    ],
    simStatus: "partial",
  },
  {
    id: "hazards",
    aliases: ["trap", "haunt", "pit", "rune", "spear_launcher"],
    title: "Hazards (traps, haunts, environmental)",
    actions: [],
    brief:
      "Catalog hazards placed on cells; enter_square / end_turn_in triggers attack or save damage. Tag hazardous still deals static pathing damage.",
    verbose: [
      "ingest-hazards → data/hazards/{index,catalog}.json (AoN category:hazard).",
      "Fixture field hazards[]: { hazardId, cells[], disabled? }.",
      "Classic map seeds Hidden Pit + Spear Launcher when hazards omitted.",
      "Complex hazards (Spinning Blade Pillar) use end_turn_in; once:true disarms after trigger.",
    ],
    simStatus: "partial",
  },
  {
